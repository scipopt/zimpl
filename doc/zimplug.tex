% $Id: zimplug.tex,v 1.6 2003/10/16 07:42:58 bzfkocht Exp $
%* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
%*                                                                           *
%*   File....: zimplug.tex                                                   *
...
-D {\em name=val} & Sets the parameter \emph{par} to the secified
                  value. This is equivalent with having this line in the
                  \zimpl programm: \code{param param par:=value}.\\
%\hline
...
variables. Als these variables have to be bounded integer or binary
...
allowed. Also combination of serveral terms with \code{and},
...
\code{else} may include bounded continous variables. 
\begin{center}
...
subto c3: forall <i> in I : vif x[i] >= 2 then x[i + 1] <= 4 end;
\end{verbatim}
}
...
arguments of these functions have to be linear terms consisting out of
...
\item[137 Upper bound for var \code{xxx} set to -infinity -- ignored] (warning)\ \\
  ...
  been appeared. This makes no sense and is therefore ignored.
\item[138 Priority/Startval for continous var \code{xxx} ignored] (warning)\ \\
...
% set.c
%
\item[164 Dublicate element \code{xxx} for set rejected] (warning)\ \\
   An element was added to a set that was already in it.
\item[165 Comparison of differen dimension sets] (warning)\ \\
...
\item[177 Boolean constraint not all integer]\ \\
   No continous (real) variables are allowed in a boolean constraint.
...
\item[180 Conditional constraint always true due to bounds] (warning)\ \\
   ...
   This due to the bounds of the variables involved.
...
\item[183 vabs term not all integer]\ \\
   ...
   Due to numerical reasons continuos variables are not allowed as
