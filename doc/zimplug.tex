...
%
\noindent
{\bfseries Some topics are not yet covered in this manual.
Notably, the use of non-linear constraints and constraint attributes.
...
-t \emph{format} & Selects the output format. Can be either \code{lp},
		  ...
		  same as \code{lp} but with rows and columns randomly
                  permuted. The permutation is depending on the \emph{seed}.\\
...
\code{random(m,n)}               &pseudo random number      &$\in[m,n]$\\
\code{ord(A,n,c)}                &ordinal                   &c-th component of the n-th\\ 
                                 &                          & element of set $A$.\\
\code{length(s)}                 &length of a string        &number of chracters in $s$\\
\code{if a then b}               &                          &\\
...
If \code{begin} is negative, counting starts
...
\code{union <i>}&\\
\code{ in I: S}&\raisebox{1ex}[0cm][0cm]{union} 
               &\raisebox{1ex}[0cm][0cm]{$\bigcup_{i\in I}S_i$} \\
\code{A inter B}   &intersection & $\{x\mid x\in A\wedge x\in B\}$\\
\code{inter <i>}&\\
\code{ in I: S}&\raisebox{1ex}[0cm][0cm]{intersection} 
               &\raisebox{1ex}[0cm][0cm]{$\bigcap_{i\in I}S_i$} \\
...
\code{argmin <i>}&\\
\code{ in I : e(i)} & minimum argument 
   &$\argmin_{i\in I} e(i)$\\
...
\code{argmax <i>}&\\
\code{ in I : e(i)} & maximum argument 
   &$\argmax_{i\in I} e(i)$\\
