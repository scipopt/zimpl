...
\begin{equation}
  \begin{array}{rll}
    \min& \displaystyle\sum_{(i,j) \in A} x_{ij} x_{ij}\\
    & \displaystyle\sum_{(iv)\in \delta^-(v)} x_{iv} = \sum_{(vi)\in \delta^+(v)
      x_{vi}} \mbox{for all }v \in V\setminus\{s,t\} \\
    & \displaystyle\sum_{(si)\in \delta^+(s) x_{si}} \\
...
at the end of the string. \code{length} is the number of chacaters to extract 
...
Boolean expresseion can also be in the \emph{then} or \emph{else} part of an 
\emph{if} expression.
...
tuples. Each tuple is unique within a set. The sets in \zimpl are all
ordered, but there is no particular order of the tuples.  Sets are
...
a tuple is enclosed in angle brackets $<$ and $>$, \eg
\code{$<$1,2,"x"$>$}. The components are separated by commas.  If
...
\noindent
Assignments need not to be complete.  In the example, no value is
...
[\code{fs} \emph{s}] [\code{match} \emph{s}] [\code{comment} \emph{s}]
...
\section{Models}
%------------------------------------------------------------------------------
In this section we use the machinery developed up to now in order to
...
(\url{http://scip.zib.de}) with linked in \zimpl is able to do so.
In all other cases the variable is treated as a continous variable. 
