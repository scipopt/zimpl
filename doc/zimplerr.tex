...
\item[302 SOS priority has to be integral] (warning)\ \\
   ...
   %
\item[401 RHS unequal to 1 can't be converted to QUBO (yet)]\ \\
   Currently, only constraints with an right hand side of 1 can be
   automatically converted into a QUBO.
\item[402 Less equal, greater equal and range can't be converted to
  QUBO (yet)]\ \\
  Currently, only equality constants can be automatically converted
  into a QUBO.
...
\item [404 Non linear expressions can't be converted to QUBO]\ \\
  ...
  automatically converted into a QUBO. 
\item [405 Constraints with coefficients != 1 can't be converted to
  QUBO]\ \\
  Currently, no coefficients for the variables are allowed in an
  constraints that should be automatically converted into a QUBO.  
