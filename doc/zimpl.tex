% $Id: zimpl.tex,v 1.11 2003/05/04 07:27:32 bzfkocht Exp $
%* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
%*                                                                           */
%*   File....: zimpl.tex                                                     */
...
\rcsInfo $Id: zimpl.tex,v 1.11 2003/05/04 07:27:32 bzfkocht Exp $
%
\parindent0ex
...
statement. {\em cf} will use the name given with the \code{subto},
then a $1\ldots n$ number like in \emph{cm} and then append all the
local variables from the forall statements.\\
%\hline
-m&Writes the output file with unmangled variable names. This file
   can usually not be optimizized because the format is invalid.\\
...
\code{if b the a else b}         &conditional               &\\
\end{tabular}
...
\eg, \code{"Hallo"}. At this time no further functions or operators 
...
be once in a set. Sets start and end with \code{\{} and \code{\}},
resp.
