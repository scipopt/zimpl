% $Id: zimpl.tex,v 1.15 2003/09/05 13:53:55 bzfkocht Exp $
%* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
%*                                                                           */
%*   File....: zimpl.tex                                                     */
...
\rcsInfo $Id: zimpl.tex,v 1.15 2003/09/05 13:53:55 bzfkocht Exp $
%
\parindent0ex
...
\author{Thorsten Koch}
\date{\small for Version 2.00\\21. August 2003}
\maketitle
\vfill
\begin{abstract}
...
\item[106 Empty LHS, contraint trivally violated]\ \\
...
\item[107 Range must be $l\leq x\leq u$, or $u \geq x\geq l$]\ \\
   ...
   on both sides.
\item[108 Empty Term with nonempty LHS/RHS, contraint trivally
   violated]\ \\
...
\item[109 LHS/RHS contradiction, contraint trivally violated]\ \\
...
\item[119 Union of incompatible sets]\ \\
   To unite two sets, both must have the same dimension tuples.
...
\item[125 ``step'' value \code{xxx} in range too big or not an
   ...
   absolute value of less than two billion.
\item[126 ``step'' value \code{xxx} in range not moving from
   \code{yyy} to \code{zzz}]\ \\
   If the given ``step'' value for the generation of a set is
   repeatedly added to the ``from'' value, the ``upto'' value 
