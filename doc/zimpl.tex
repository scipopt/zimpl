% $Id: zimpl.tex,v 1.10 2003/03/18 09:37:03 bzfkocht Exp $
%* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
%*                                                                           */
%*   File....: zimpl.tex                                                     */
...
\rcsInfo $Id: zimpl.tex,v 1.10 2003/03/18 09:37:03 bzfkocht Exp $
%
\parindent0ex
...
\author{Thorsten Koch}
\date{\small for Version 1.05\\18. March 2003}
\maketitle
\vfill
\begin{abstract}
...
\ampl today.
On the other hand, having the source code of a program has its atvantages. The
...
& Defaults to the name of the first input file striped of the path and
extension.\\ 
-p {\em filter} & The output is piped through a filter. A \%s in the
string is replaced by the output filename. For example 
\code{-p "gzip -c >\%s.gz"} would compress all the output files.\\
-n {\em cform}& Select the format for the generation of constraint
names. Can be either {\em cm} which will number them $1\ldots n$ with
a `c' in front. {\em cn} will use the name supplied in the
\code{subto} statement and number them $1\ldots n$ within the
statement. {\em cf} will use the name of given with the \code{subto},
then a $1\ldots n$ number like in \emph{cm} and then append all the
local variables from the forall statements.\\
%\hline
...
$^\wedge$, \code{+}, \code{-}, \code{*}, \code{/}, \code{mod},
\code{div}, \code{abs}, \code{floor}, \code{ceil}, 
\code{log} (logarithm with base 10),
\code{ln} (logarithm with base $e$), \code{exp} ($e^x$), \code{$!$} (factorial). 
$^\wedge$ stands for \emph{to the
...
\code{random(a, b)} generates a random number between $a$ and $b$.
...
&(default $s = 1$)&\\ 
$\mbox{proj}(A, t)$& Projection $t=(e_1,\ldots,e_n)$& 
The new set will consist of $n$-tuples, with the
$i$-th component beeing the $e_i$-th component of $A$.\\
\hline
\end{tabular}
...
\begin{tabular}{llp{5cm}}
\hline
$\mbox{powerset}(A)$& Generates all subsets of $A$&$\{x | x\subseteq A\}$\\
$\mbox{subset}(A,n)$& Generates all subsets of $A$\\
                    & with $n$ elements&$\{x | x\subseteq A\wedge |x|=n\}$\\
$\mbox{indexset}(A)$&The index set of $A$&$\{1\ldots |A|\}$\\
\hline
\end{tabular}
...
\item[Type error, expected xxx got yyy]\ \\
 ...
 in \code{code.h}.
\item[Duplicate constraint name "xxx"]\ \\
 The name given to a \code{subto} statement was allready used with
 another \code{subto} statement. Either change the name, or use 
 \code{-n cm} or \code{-n cf}.
\end{description}
