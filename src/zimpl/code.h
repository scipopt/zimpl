...
/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
/*
 * Copyright (C) 2001-2021 by Thorsten Koch <koch@zib.de>
 * 
...
extern CodeNode*    code_new_strg(const char* strg) expects_NONNULL returns_NONNULL;
//lint -sem(        code_new_name, custodial(1), 1p == 1, @P >= malloc(1))
extern CodeNode*    code_new_name(const char* name) expects_NONNULL returns_NONNULL;
...
extern Inst         code_get_inst(const CodeNode* node) expects_NONNULL is_PURE;
...
extern void         code_errmsg(const CodeNode* node) expects_NONNULL;
...
extern CodeNode*    code_get_child(const CodeNode* node, int no) expects_NONNULL returns_NONNULL;
//lint -sem(        code_get_numb, 1p == 1, @p) 
extern const Numb*  code_get_numb(CodeNode* node) expects_NONNULL returns_NONNULL;
//lint -sem(        code_get_strg, 1p == 1, @p) 
extern const char*  code_get_strg(CodeNode* node) expects_NONNULL returns_NONNULL;
//lint -sem(        code_get_name, 1p == 1, @p) 
extern const char*  code_get_name(CodeNode* node) expects_NONNULL returns_NONNULL;
...
extern const Tuple* code_get_tuple(CodeNode* node) expects_NONNULL returns_NONNULL;
//lint -sem(        code_get_set, 1p == 1, @p) 
extern const Set*   code_get_set(CodeNode* node) expects_NONNULL returns_NONNULL;
//lint -sem(         code_get_idxset, 1p == 1, @p) 
extern const IdxSet* code_get_idxset(CodeNode* node) expects_NONNULL returns_NONNULL;
//lint -sem(        code_get_entry, 1p == 1, @p)
extern const Entry* code_get_entry(CodeNode* node) expects_NONNULL returns_NONNULL;
//lint -sem(        code_get_term, 1p == 1, @p)
extern const Term*  code_get_term(CodeNode* node) expects_NONNULL returns_NONNULL;
...
extern const List*  code_get_list(CodeNode* node) expects_NONNULL returns_NONNULL;
...
extern const RDef*  code_get_rdef(CodeNode* node) expects_NONNULL returns_NONNULL;
//lint -sem(        code_get_rpar, 1p == 1, @p)
extern const RPar*  code_get_rpar(CodeNode* node) expects_NONNULL returns_NONNULL;
...
extern const Bound* code_get_bound(CodeNode* node) expects_NONNULL returns_NONNULL;
...
extern void         code_value_strg(CodeNode* node, const char* strg) expects_NONNULL;
//lint -sem(        code_value_name, inout(1), 1p == 1, custodial(2), 2p == 1) 
extern void         code_value_name(CodeNode* node, const char* name) expects_NONNULL;
...
extern void         code_copy_value(CodeNode* dst, const CodeNode* src) expects_NONNULL;

//lint -sem(        code_eval_child, 1p == 1, chneg(2), @p == 1) 
extern CodeNode*    code_eval_child(const CodeNode* node, int no) expects_NONNULL returns_NONNULL;
//lint -sem(        code_eval_child_numb, 1p == 1, chneg(2), @p == 1) 
extern const Numb*  code_eval_child_numb(const CodeNode* node, int no) expects_NONNULL returns_NONNULL;
//lint -sem(        code_eval_child_strg, 1p == 1, chneg(2), @p)  
extern const char*  code_eval_child_strg(const CodeNode* node, int no) expects_NONNULL returns_NONNULL;
//lint -sem(        code_eval_child_name, 1p == 1, chneg(2), @p) 
extern const char*  code_eval_child_name(const CodeNode* node, int no) expects_NONNULL returns_NONNULL;
//lint -sem(        code_eval_child_tuple, 1p == 1, chneg(2), @p == 1)  
extern const Tuple* code_eval_child_tuple(const CodeNode* node, int no) expects_NONNULL returns_NONNULL;
//lint -sem(        code_eval_child_set, 1p == 1, chneg(2), @p == 1)  
extern const Set*   code_eval_child_set(const CodeNode* node, int no) expects_NONNULL returns_NONNULL;
//lint -sem(         code_eval_child_idxset, 1p == 1, chneg(2), @p == 1)  
extern const IdxSet* code_eval_child_idxset(const CodeNode* node, int no) expects_NONNULL returns_NONNULL;
//lint -sem(        code_eval_child_entry, 1p == 1, chneg(2), @p == 1)  
extern const Entry* code_eval_child_entry(const CodeNode* node, int no) expects_NONNULL returns_NONNULL;
//lint -sem(        code_eval_child_term, 1p == 1, chneg(2), @p == 1)  
extern const Term*  code_eval_child_term(const CodeNode* node, int no) expects_NONNULL returns_NONNULL;
//lint -sem(        code_eval_child_size, 1p == 1, chneg(2), chneg(@)) 
extern int          code_eval_child_size(const CodeNode* node, int no) expects_NONNULL;
//lint -sem(        code_eval_child_bool, 1p == 1, chneg(2)) 
extern bool         code_eval_child_bool(const CodeNode* node, int no) expects_NONNULL;
//lint -sem(        code_eval_child_list, 1p == 1, chneg(2), @p == 1)  
extern const List*  code_eval_child_list(const CodeNode* node, int no) expects_NONNULL returns_NONNULL;
//lint -sem(        code_eval_child_varclass, 1p == 1, chneg(2)) 
extern VarClass     code_eval_child_varclass(const CodeNode* node, int no) expects_NONNULL;
//lint -sem(        code_eval_child_contype, 1p == 1, chneg(2)) 
extern ConType      code_eval_child_contype(const CodeNode* node, int no) expects_NONNULL;
//lint -sem(        code_eval_child_rdef, 1p == 1, chneg(2), @p == 1)  
extern const RDef*  code_eval_child_rdef(const CodeNode* node, int no) expects_NONNULL returns_NONNULL;
//lint -sem(        code_eval_child_rpar, 1p == 1, chneg(2), @p == 1)  
extern const RPar*  code_eval_child_rpar(const CodeNode* node, int no) expects_NONNULL returns_NONNULL;
//lint -sem(        code_eval_child_bits, 1p == 1, chneg(2)) 
extern unsigned int code_eval_child_bits(const CodeNode* node, int no) expects_NONNULL;
//lint -sem(        code_eval_child_symbol, 1p == 1, chneg(2), @p == 1)  
extern Symbol*      code_eval_child_symbol(const CodeNode* node, int no) expects_NONNULL returns_NONNULL;
//lint -sem(        code_eval_child_define, 1p == 1, chneg(2), @p == 1)  
extern Define*      code_eval_child_define(const CodeNode* node, int no) expects_NONNULL returns_NONNULL;
//lint -sem(        code_eval_child_bound, 1p == 1, chneg(2), @p == 1)  
extern const Bound* code_eval_child_bound(const CodeNode* node, int no) expects_NONNULL returns_NONNULL;
