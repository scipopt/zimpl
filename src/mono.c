...
Mono* mono_new(const Numb* coeff, const Entry* entry, const Numb* power)
{
   ...
   mono->first.power = numb_copy(power);
...
Mono* mono_copy(const Mono* mono)
{
   ...
   mnew = mono_new(mono->coeff, mono->first.entry, mono->first.power);
   
   for(e = mono->first.next; e != NULL; e = e->next)
      mono_add_elem(mnew, e->entry, e->power);
...
unsigned int mono_hash(const Mono* mono)
{
   ...
   for(e = &mono->first; e != NULL; e = e->next)
      hcode += numb_hash(e->power) + (unsigned int)entry_get_var(e->entry);

   return DISPERSE(hcode);
}
...
Bool mono_is_linear(const Mono* mono)
{
   ...
   return numb_equal(mono->first.power, numb_one());
}

int mono_get_count(const Mono* mono)
{
   assert(mono_is_valid(mono));
